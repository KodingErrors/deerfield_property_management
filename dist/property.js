...
if (!item) {
  ...
  root.innerHTML = `
    ...
    <section class="detail-columns">
      <article><p class="eyebrow">Property overview</p><h2>Current listing details.</h2><p>${escapeHtml(item.description || `${labels[item.type]} property in ${item.city}, Ontario, represented in Deerfield Brokerage’s public portfolio.`)}</p>${knownFeatures.length ? `<div class="feature-list">${knownFeatures.map((feature) => `<span>✓ ${escapeHtml(feature)}</span>`).join("")}</div>` : '<p class="data-caveat">Detailed building features are not published in the source portfolio and should be confirmed with the broker.</p>'}</article>
