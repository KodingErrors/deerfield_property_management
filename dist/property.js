...
import { DETAIL_LABELS, formatDetail } from "./seed-details.js";
...
if (!item) {
  ...
  const specRows = DETAIL_LABELS
    .map(([key, label]) => [label, formatDetail(key, item.details[key])])
    .filter(([, value]) => value !== null);
  const specList = specRows.length
    ? `<dl class="spec-list">${specRows.map(([label, value]) =>
